...
/* Write a function called async.
  ...
  Call setTimeout on a function which sets the variable foo (above) to 'bar' and then resolves the promise when setTimeout completes.
*/
function async() {
  var deferred = $q.defer();
  deferred.resolve(setTimeout((function() {
    foo = 'bar';
  })(), 7000))
...
function context1(myFn, context, param1, param2) {
  var result = myFn.call(context, param1, param2);
  return result;
}
...
function context2(myFn, context, params) {
  var result = myFn.apply(context, params);
  return result;
}
...
function subway(personName) {
  var ingredients = [];
  return function addIngredient(added) {
    ingredients += added;
    return {
...
function compareValues(par1, par2) {
  var par1Type = (typeof par1);
  var par2Type = (typeof par2);
  if ((par1Type === par2Type) && (par1 === par2)) {
    return "Exact match";
  } else if ((par1Type !== par2Type) && (par1.toString() === par2.toString())) {
